...
# Custom field setting with display preferences and parent association details.
...
public type AttachmentSingleResponseOk record {|
    *http:Ok;
    AttachmentSingleResponse body;
|};

# Request body wrapper for creating a new task.
public type TasksBody record {
    # Request payload for creating a new task with workspace, followers, and associations.
...
# Request payload for creating an attachment on a task, project, or project brief.
public type AttachmentRequest record {
    ...
    "asana"|"external" resourceSubtype?;
...
public type TaskBase_external record {
    # The external system's unique identifier for this task.
    string gid?;
    # Custom data blob stored by an external application.
    string data?;
};
...
# Request object specifying project and position for adding a task to a project.
...
# Request body wrapper for creating a new tag.
public type TagsBody record {
    # Request object for creating a new tag in a workspace.
...
# Base project object containing core project properties and settings.
public type ProjectBase record {
    *ProjectCompact;
    ...
    # Deprecated project status; use current_status_update instead.
...
# Request schema for creating or updating a project with all configurable properties.
...
public type TaskstaskGidBody record {
    # Request schema for updating an existing task with optional custom type fields.
...
# Base representation of a custom field with configuration properties.
...
# Response containing a paginated list of compact project records.
public type ProjectCompactListResponse record {
    # Pagination information for retrieving the next page of results.
    @jsondata:Name {value: "next_page"}
    NextPage? nextPage?;
    # Array of compact project objects returned by the request.
    ProjectCompact[] data?;
};

# Base task schema with full task details including dates, status, and metadata.
...
# User response with base properties, profile photo URLs, and email address.
...
public type ErrorResponse record {
    # Array of error objects containing details about the API error.
    Error[] errors?;
};

# Paginated response containing a list of compact task objects.
...
# Response object containing task search results.
...
# Request object for creating a new tag in a workspace.
public type TagCreateRequest record {
    *TagBaseRequest;
    # Gid of an object
    string workspace?;
...
# Response wrapper containing a single project's data.
public type ProjectSingleResponse record {
    # Complete project details including ownership, status, and associated metadata.
...
# Complete attachment details including URLs, hosting info, and parent task reference.
public type AttachmentResponse record {
    *AttachmentBase;
    ...
    record {*TaskCompact; @jsondata:Name {value: "resource_subtype"}
        string? resourceSubtype?;} parent?;
...
public type TeamSingleResponse record {
    # Full team representation including visibility, permissions, and organization details.
    TeamResponse data?;
};
...
public type ProjectResponse record {
    *ProjectBase;
    ...
    @jsondata:Name {value: "completed_at"}
    string? completedAt?;
    # The workspace or organization this project belongs to (create-only).
    record {*WorkspaceCompact;} workspace?;
...
# Full representation of a project status update with author and timestamps.
...
# Paginated response containing a list of compact tag objects.
...
# Response wrapper containing a single attachment resource.
public type AttachmentSingleResponse record {
    # Complete attachment details including URLs, hosting info, and parent task reference.
...
# Response containing a paginated list of attachments.
public type AttachmentListResponse record {
    # Pagination information for retrieving the next page of results.
    @jsondata:Name {value: "next_page"}
    NextPage? nextPage?;
    # Array of compact attachment objects.
    AttachmentCompact[] data?;
};

# Response containing a single story object with full details.
public type StorySingleResponse record {
    # Full representation of a story with change tracking and interaction details.
    StoryResponse data?;
};
...
public type StoryResponse record {
    *StoryBase;
    ...
    # The previous date values before the change.
    ...
    # The type of story, either a user comment or system-generated event.
    "comment"|"system" 'type?;
    ...
    string? newName?;
    # The custom field that was modified in this story event.
    @jsondata:Name {value: "custom_field"}
    ...
    # The new date values after the change was made.
    ...
    boolean isEdited?;
    # The new value of a date custom field after modification.
    ...
    @jsondata:Name {value: "created_by"}
    UserCompact createdBy?;
    # The object this story is associated with, currently only tasks.
    ...
    Preview[] previews?;
    # The new enum option value after the change.
