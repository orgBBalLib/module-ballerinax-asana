...
# Full custom field setting record with display and parent information.
public type CustomFieldSettingResponse record {
    *CustomFieldSettingBase;
    ...
    # Deprecated: Use `parent` field. The project for this custom field setting.
...
# Response containing a paginated list of section objects in compact format.
public type SectionCompactListResponse record {
    ...
    NextPage? nextPage?;
    # Array of section objects in compact format.
    SectionCompact[] data?;
};
...
# Request object for creating an attachment on a task, project, or project brief.
...
public type TaskResponse record {
    *TaskBase;
    ...
    # The selected status option for the task's custom type.
    ...
    # The user assigned to complete this task, or null if unassigned.
...
public type TaskBase_external record {
    # A unique identifier for the external data reference.
    string gid?;
...
public type ProjectBase record {
    *ProjectCompact;
    ...
    string htmlNotes?;
    # The most recent status update posted to this project.
...
# Base custom field object with configuration and formatting properties.
...
# A full workspace object with organization status and email domains.
...
public type ProjectCompactListResponse record {
    ...
    @jsondata:Name {value: "next_page"}
    NextPage? nextPage?;
    # Array of compact project objects returned by the query.
    ProjectCompact[] data?;
};

# Base task object with detailed properties including dates, status, and metadata.
...
public type ErrorResponseNotImplemented record {|
    *http:NotImplemented;
    ErrorResponse body;
|};

# Response containing a paginated list of story objects in compact format.
public type StoryCompactListResponse record {
    # Pagination information for retrieving the next set of stories.
    @jsondata:Name {value: "next_page"}
    NextPage? nextPage?;
    # Array of story objects in compact format.
    StoryCompact[] data?;
};

# User response with base fields, profile photo, and email address.
...
# Base tag schema with name, notes, and color properties.
...
# Response containing a paginated list of compact user objects.
...
public type ProjectSingleResponse record {
    # Complete project details returned from API responses.
...
# Complete attachment object with base properties and extended metadata.
...
# Request body wrapper for team creation operations.
...
public type TeamSingleResponse record {
    # Full team object with visibility settings, access controls, and organization details.
    TeamResponse data?;
};
...
# Request body wrapper for updating a story.
...
public type ProjectResponse record {
    *ProjectBase;
    ...
    # The project template from which this project was created, if applicable.
...
public type ErrorResponseFailedDependency record {|
...
# A project status update with author and timestamp information.
public type ProjectStatusResponse record {
    *ProjectStatusBase;
    ...
    @jsondata:Name {value: "modified_at"}
    string modifiedAt?;
    # The user who created this project status.
    @jsondata:Name {value: "created_by"}
    UserCompact createdBy?;
};
...
public type ErrorResponseGatewayTimeout record {|
    *http:GatewayTimeout;
    ErrorResponse body;
|};
...
# Error object containing message, help link, and error phrase details.
...
public type StorySingleResponse record {
    # Complete story object with all properties including change tracking and metadata.
    StoryResponse data?;
};
...
public type ErrorResponsePaymentRequired record {|
    *http:PaymentRequired;
    ErrorResponse body;
|};

# Complete user record with custom fields and workspace access information.
...
public type StoryResponse record {
    *StoryBase;
    ...
    boolean hearted?;
    # The new section a task was moved to.
    ...
    string? newName?;
    # The custom field that was changed in this story event.
    @jsondata:Name {value: "custom_field"}
    ...
    Preview[] previews?;
    # The new enum option value after a custom field change.
