...
# Custom field setting with display preferences and parent context.
public type CustomFieldSettingResponse record {
    *CustomFieldSettingBase;
    ...
    # Deprecated. The project this custom field setting refers to. Use `parent` instead.
...
# Request schema for creating or updating a team with visibility and access settings.
...
public type TasksBody record {
    # Request body containing all fields required to create a new task.
...
public type SectionCompactListResponse record {
    ...
    NextPage? nextPage?;
    # Array of compact section objects returned by the query.
    SectionCompact[] data?;
};
...
public type TaskResponse record {
    *TaskBase;
    # The parent task if this is a subtask, or null otherwise.
    record {*TaskCompact;} parent?;
    # The workspace this task belongs to. Set at creation time only.
    ...
    # The custom type status option applied to this task.
    ...
    string permalinkUrl?;
    # The section in the assignee's My Tasks list containing this task.
    @jsondata:Name {value: "assignee_section"}
...
# Request object containing the tag GID to add to a task.
...
# Response wrapper containing an empty data payload.
public type EmptyDataResponse record {
    # An empty object. Some endpoints do not return an object on success. The success is conveyed through a 2-- status code and returning an empty object
    EmptyResponse data?;
};

# Schema representing a project creation or update request with all configurable fields.
...
# Base custom field object with configuration and display properties.
...
# A full workspace object with organizational and domain information.
...
# Response wrapper containing a paginated list of compact project records.
...
# Base task object with full details including dates, status, and metadata.
...
public type StoryCompactListResponse record {
    ...
    @jsondata:Name {value: "next_page"}
    NextPage? nextPage?;
    # Array of compact story objects returned by the query.
    StoryCompact[] data?;
};

# User response containing base properties, profile photo URLs, and email address.
...
# Base tag schema with descriptive properties and color settings.
...
public type ErrorResponse record {
    # Array of error objects containing details about the failures.
    Error[] errors?;
};
...
public type WorkspaceCompactListResponse record {
    # Pagination information for retrieving the next page of results.
    @jsondata:Name {value: "next_page"}
    NextPage? nextPage?;
...
public type ProjectResponse record {
    *ProjectBase;
    ...
    @jsondata:Name {value: "completed_at"}
    string? completedAt?;
    # The workspace or organization this project belongs to. Set at creation only.
    record {*WorkspaceCompact;} workspace?;
    ...
    @jsondata:Name {value: "permalink_url"}
    string permalinkUrl?;
    # The user who marked this project complete, or null if not completed.
    @jsondata:Name {value: "completed_by"}
    UserCompact completedBy?;
};
...
public type ProjectStatusResponse record {
    *ProjectStatusBase;
    ...
    @jsondata:Name {value: "modified_at"}
    string modifiedAt?;
    # The user who originally created this project status.
    @jsondata:Name {value: "created_by"}
    UserCompact createdBy?;
};
...
# Error object containing details about API request failures.
...
public type StorySingleResponse record {
    # A complete story object with all properties including change tracking details.
    StoryResponse data?;
};
...
public type StoryResponse record {
    *StoryBase;
    ...
    Preview[] previews?;
    # The new enum option value after modification.
