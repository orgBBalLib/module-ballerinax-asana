...
# Complete custom field setting with display preferences and parent context.
...
public type TaskResponse record {
    *TaskBase;
    ...
    # The workspace this task belongs to. Set only at creation time.
...
public type TaskBase_external record {
    ...
    string gid?;
    # Arbitrary external data blob stored with the task.
    string data?;
};
...
public type TaskstaskGidBody record {
    # Request object for updating an existing task.
...
# Base custom field object with configuration properties for workspace-level fields.
...
# A full workspace object with organization status and email domain details.
...
# Response containing a paginated list of tags.
...
# Response object containing a paginated list of sections.
public type SectionListResponse record {
...
# Response object containing a list of compact team records with pagination.
public type TeamCompactListResponse record {
    # Pagination information for retrieving the next set of results.
    @jsondata:Name {value: "next_page"}
    NextPage? nextPage?;
    # Array of compact team objects.
    TeamCompact[] data?;
};

# Response object containing a paginated list of compact project records.
public type ProjectCompactListResponse record {
    # Pagination information for retrieving the next page of results.
    @jsondata:Name {value: "next_page"}
    NextPage? nextPage?;
    # Array of compact project records returned by the request.
    ProjectCompact[] data?;
};

# Extended task representation with detailed properties including dates and status.
public type TaskBase record {
    *TaskCompact;
    ...
    int numHearts?;
    # The user who completed this task, or null if incomplete.
    ...
    # The user who assigned this task, or null if unspecified.
...
# User response containing base fields, profile photo URLs, and email address.
...
# Response object containing a paginated list of stories.
public type StoryListResponse record {
...
# Base tag schema with compact properties, notes, and color.
...
public type ErrorResponse record {
    # Array of error objects containing details about the failed request.
    Error[] errors?;
};

# Response containing a paginated list of compact tasks.
public type TaskCompactListResponse record {
    # Pagination information for retrieving the next page of results.
    @jsondata:Name {value: "next_page"}
    NextPage? nextPage?;
    # Array of compact task objects returned by the query.
    TaskCompact[] data?;
};
...
# Request body for adding followers to a goal.
public type GoalGidAddFollowersBody record {
    # Request body for adding followers to a task or goal.
...
public type TeamSingleResponse record {
    # A complete team object with visibility, access controls, and organization details.
    TeamResponse data?;
};
...
# Response object containing a single section's details.
...
# Error object containing details about an API error response.
...
public type TaskSingleResponseOk record {|
    *http:Ok;
    TaskSingleResponse body;
|};

# Response object containing a single story's details.
public type StorySingleResponse record {
    # A complete story object with all details including changes, reactions, and metadata.
    StoryResponse data?;
};
