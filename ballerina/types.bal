...
# Request object for adding a supporting relationship to a goal.
...
# Response representing various membership types across Asana resources.
...
public type PortfolioBase record {
    ...
    boolean archived?;
    # The localized start date for the portfolio
    @jsondata:Name {value: "start_on"}
    string startOn?;
    # The localized due date for the portfolio
...
# Response object for custom field settings.
...
# Additional filter criteria for webhook events.
public type FiltersFiltersAllOf213 record {
    # Additional filter options
    record {} additionalFilters?;
};

# Combined filter criteria for updating webhook event subscriptions.
...
# Request object for triggering a rule with specified resource and action data.
...
# Base tag representation with core properties.
...
# Compact representation of a project membership with resource type information.
...
public type CustomFieldCompact record {
    ...
    # The name of the custom field
    string name?;
    # A string representation for the value of the custom field. Integrations that don't require the underlying type-specific value can use this to read and write custom field values
